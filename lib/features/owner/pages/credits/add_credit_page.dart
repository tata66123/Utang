...
class _AddCreditPageState extends State<AddCreditPage> {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        if (widget.customer != null) {
          ...
          // Find existing customer (coming from main add credit)
          final customerName = _customerNameController.text.trim();
          
          // Check if customer exists in database (Firebase + SQLite)
          final customerExists = await _store.customerExists(customerName);
          if (!customerExists) {
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Customer does not exist. Please register the customer first.'),
          ...
          // Customer exists, get or create customer record
          customer = await _store.getOrCreateCustomerByUsername(customerName);
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              Builder(
                builder: (context) {
                  ...
                  if (widget.customer != null) {
                    ...
                    final name = _customerNameController.text.trim().toLowerCase();
                    try {
                      currentCustomer = _store.state.customers.firstWhere(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: Text('Credit Date: ${_formatDate(_selectedDate)}'),
                          ),
                          TextButton(
                            onPressed: _pickDate,
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: Text('Due Date: ${_dueDate == null ? 'None' : _formatDate(_dueDate!)}'),
                          ),
                          TextButton(
              ...
              if (widget.customer != null || _customerNameController.text.isNotEmpty) ...[
                const Text(
  ...
  Widget _buildRecentCredits() {
    ...
    if (widget.customer != null) {
      // Customer is pre-selected (coming from customers page)
      customer = widget.customer!;
    } else if (_customerNameController.text.isNotEmpty) {
      // Customer name is typed (coming from main add credit)
      final customerName = _customerNameController.text.trim().toLowerCase();
      customer = _store.state.customers.firstWhere(
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Amount: ₱${credit.amount.toStringAsFixed(2)}'),
                Text('Balance: ₱${credit.balance.toStringAsFixed(2)}'),
